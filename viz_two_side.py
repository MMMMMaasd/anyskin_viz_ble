...
def notification_handler(sender, data):
    global data_record_black, data_record_white, baseline_data_black, baseline_data_white
...
async def visualize_ble(viz_mode="3axis", scaling=7.0, record=False):
    ...
    def visualize_data(data_black, data_white):
        ...
        for magid, chip_location in enumerate(chip_locations):
            if viz_mode == "magnitude":
                pygame.draw.circle(
                    window, (255, 83, 72), (chip_location[0] + 0, chip_location[1]), data_mag_black[magid] / scaling
                )
            elif viz_mode == "3axis":
                ...
                pygame.draw.circle(
                    ...
                    np.abs(data_black[magid, -1]) / scaling,
                    width_black,
                )
                ...
                arrow_end_black = (
                    chip_location[0] + data_xy_black[0] / scaling + 0,
                    chip_location[1] + data_xy_black[1] / scaling,
                )
                pygame.draw.line(window, (0, 255, 0), arrow_start, arrow_end_black, 2)
                
        for magid, chip_location in enumerate(chip_locations):
                if viz_mode == "magnitude":
                    pygame.draw.circle(
                        window, (255, 83, 72), (chip_location[0] + 400, chip_location[1]), data_mag_white[magid] / scaling
                    )
                elif viz_mode == "3axis":
                    ...
                    pygame.draw.circle(
                        ...
                        np.abs(data_white[magid, -1]) / scaling,
                        width_white,
                    )
                    ...
                    arrow_end_white = (
                        chip_location[0] + data_xy_white[0] / scaling + 400,
                        chip_location[1] + data_xy_white[1] / scaling,
                    )
                    pygame.draw.line(window, (0, 255, 0), arrow_start, arrow_end_white, 2)
    ...
    while running:
        ...
        if len(data_record_black) > 0:
            ...
            visualize_data((data_record_black[0] - baseline_data_black), ((data_record_white[0] - baseline_data_white)))
